...
function phantomLog(message, type = 'info') {
    const icons = { 
        info: '📝', 
        success: '✅', 
        error: '❌', 
    ...
    console.log(`${icons[type] || '📝'} ✦ PHANTOM-X ✦ ${new Date().toISOString()}: ${message}`);
}
...
phantomLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { 
        require(path.join(pluginsDir, file)); 
    } catch (e) { 
        phantomLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); 
    }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                phantomLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            phantomLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            phantomLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                phantomLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { 
                phantomLog(`Normal closure for ${number}, no restart needed.`, 'info'); 
                return; 
            }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                phantomLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    phantomLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); 
                }
            } else {
                phantomLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
        if (connection === 'open') { 
            restartAttempts = 0; 
...
async function arslanPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `phantom_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            phantomLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                phantomLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            fs.ensureDirSync(sessionPath);
            fs.writeFileSync(path.join(sessionPath, 'creds.json'), JSON.stringify(existingSession, null, 2));
            phantomLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }

        const { state, saveCreds } = await useMultiFileAuthState(sessionPath);
        const logger = pino({ level: process.env.NODE_ENV === 'production' ? 'fatal' : 'debug' });

        const phantomStore = createPhantomStore();
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await phantomStore.loadMessage(key.remoteJid, key.id);
                return msg && msg.message ? msg.message : { conversation: 'PHANTOM-X' };
            }
        });

        socketCreationTime.set(sanitizedNumber, Date.now());
        activeSockets.set(sanitizedNumber, conn);
        phantomStore.bind(conn.ev);
        ...
        if (!conn.authState.creds.registered) {
            phantomLog(`🔐 Starting NEW pairing process for ${sanitizedNumber}`, 'info');
            try {
                await delay(1500);
                const code = await conn.requestPairingCode(sanitizedNumber);
                phantomLog(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
                    res.send({ code, status: 'new_pairing' });
                }
            } catch (error) {
                phantomLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            phantomLog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                phantomLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
        ...
        conn.ev.on('messages.update', async (updates) => {
            try {
                ...
                if (userConfig.ANTIDELETE === 'true') {
                    await handleAntidelete(conn, updates, phantomStore, sanitizedNumber);
                }
            } catch (error) {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                await arslanmd(conn);
                phantomLog(`Connected: ${sanitizedNumber}`, 'success');
...
✧══════════════════════✧`
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) 
                    phantomLog(`Session logged out.`, 'error');
            }
        });
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© ᴘʜᴀɴᴛᴏᴍ-x',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN: PHANTOM-X BOY\nORG: PHANTOM-X BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© ᴘʜᴀɴᴛᴏᴍ-x' } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        try {
                            ...
                            phantomLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); 
                        }
                    }
                }
                ...
                phantomLog(`Message handler error: ${e.message}`, 'error'); 
            }
        });

    } catch (err) {
        phantomLog(`PHANTOM-X Pair error: ${err.message}`, 'error');
...
router.get('/verify-otp', async (req, res) => {
    ...
    const socket = activeSockets.get(n);
    if (socket) await socket.sendMessage(jidNormalizedUser(socket.user.id), { 
        text: `
✧══════════════════════✧
     ✦ ᴘʜᴀɴᴛᴏᴍ-χ ✦
✧══════════════════════✧
...
async function autoReconnectFromMongoDB() {
    try {
        phantomLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { 
            phantomLog('No numbers in MongoDB', 'info'); 
            return; 
        }
        for (const number of numbers) {
            if (!activeSockets.has(number)) {
        ...
        phantomLog('Auto-reconnect completed', 'success');
    } catch (e) { 
        phantomLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); 
    }
}
...
process.on('uncaughtException', (err) => {
    phantomLog(`Uncaught exception: ${err.message}`, 'error');
});
